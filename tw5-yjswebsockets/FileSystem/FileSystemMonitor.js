/*\
title: $:/plugins/joshuafontany/tw5-yjs/FileSystemMonitor.js
...
exports.after = ["YjsStartup"];
exports.platforms = ["node"];
exports.synchronous = true;

exports.startup = function() {
  if($tw.node && $tw.Yjs.settings.disableFileWatchers !== 'yes') {
    ...
    */
    $tw.Yjs.WatchFolder = function(folder, prefix) {
      // If there is no prefix set it to an empty string
      prefix = prefix || '';
      $tw.Yjs.Wikis[prefix].watchers = $tw.Yjs.Wikis[prefix].watchers || {};
      try {
        $tw.Yjs.Wikis[prefix].watchers[folder] = fs.watch(folder, function(eventType, filename) {
          filename = filename || "";
            ...
            if(err) {
              ...
              if(err.code === 'ENOENT') {
                ...
                if(['.tid', '.meta'].indexOf(fileExtension) !== -1) {
                  $tw.Yjs.DeleteTiddler(folder, filename, prefix);
                } else {
                  $tw.Yjs.logger.log('non-tiddler file deleted:', filename, {level: 3})
                }
              ...
              if(fileStats.isDirectory()) {
                $tw.Yjs.WatchFolder(itemPath, prefix)
              } else if(fileStats.isFile()) {
                const tiddlerName = Object.keys($tw.Yjs.Files[prefix]).filter(function(item) {
                  // This is to handle some edge cases I ran into while making
                  // it.
                  if(typeof item === 'string') {
                    return ($tw.Yjs.Files[prefix][item].filepath === itemPath)
                  } else {
                    return false;
                  }
                })[0];
                if(['.tid', '.meta'].indexOf(fileExtension) !== -1) {
                  ...
                  if(tiddlerObject.tiddlers[0]['title']) {
                    ...
                    const existingTiddler = $tw.Yjs.Wikis[prefix].wiki.getTiddler(title);
                    // Load the tiddler from the wiki, check if they are different (non-existent is changed)
                    if($tw.Yjs.tiddlerHasChanged(existingTiddler, {fields: tiddlerObject.tiddlers[0]})) {
                      // Rename the file
                      ...
                      if(typeof tiddlerName === 'string' && tiddlerName !== tiddlerObject.tiddlers[0].title) {
                        $tw.Yjs.logger.log('Rename Tiddler ', tiddlerName, ' to ', newTitle, {level:2});
                        // Remove the old tiddler
                        $tw.Yjs.DeleteTiddler(folder, tiddlerName + fileExtension, prefix);
                      }
          ...
          if(error.code === 'EPERM' && require('os').platform() === 'win32') {
            $tw.Yjs.logger.log('[Info] Failed to watch deleted folder.', {level:3});
            return;
          }
        });
      } catch (e) {
        $tw.Yjs.logger.error('Failed to watch folder!', e, {level:1});
      }
    }

    // TODO make this handle deleting .meta files
    $tw.Yjs.DeleteTiddler = function(folder, filename, prefix) {
      const itemPath = path.join(folder, filename);
      ...
      Object.keys($tw.Yjs.Files[prefix]).forEach(function(tiddlerName) {
        if($tw.Yjs.Files[prefix][tiddlerName].filepath === itemPath) {
          // Remove the tiddler info from $tw.Yjs.Files
          delete $tw.Yjs.Files[prefix][tiddlerName];
          // Remove the tiddler on the server
          $tw.Yjs.Wikis[prefix].wiki.deleteTiddler(tiddlerName);
    ...
    /*
      ...
      Each property in the $tw.Yjs.FolderTree object has this structure:
    ...
    */
    $tw.Yjs.WatchAllFolders = function(folderTree, prefix) {

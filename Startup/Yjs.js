/*\
title: $:/plugins/joshuafontany/yjs/Yjs.js
...
*/
class Yjs extends observable_js.Observable {
  constructor () {
    super();
    // Version
    this.version = $tw.wiki.getTiddler('$:/plugins/joshuafontany/yjs').fields.version;
    // Create a logger
    ...
    // YDocs
    this.Yversion = $tw.wiki.getTiddler('$:/plugins/joshuafontany/yjs/External/yjs/yjs.cjs').fields.version;
    this.YDocs = new Map();
  ...
  getViewableSettings (sessionId) {
    ...
    if (this.hasSession(sessionId)) {
      ...
      if (session.isLoggedIn && session.access === 'admin') {
        tempSettings.actions = $tw.Yjs.settings.actions;
        tempSettings.admin = $tw.Yjs.settings.admin;
        tempSettings.advanced = $tw.Yjs.settings.advanced;
        tempSettings.certPath = $tw.Yjs.settings.certPath;
        tempSettings.disableFileWatchers = $tw.Yjs.settings.disableFileWatchers;
        tempSettings.editions = $tw.Yjs.settings.editions;
        tempSettings.editionsPath = $tw.Yjs.settings.editionsPath;
        tempSettings.enableYjsSaver = $tw.Yjs.settings.enableYjsSaver;
        tempSettings.filePathRoot = $tw.Yjs.settings.filePathRoot;
        tempSettings['fed-wss'] = $tw.Yjs.settings['fed-wss'];
        tempSettings.httpsPort = $tw.Yjs.settings.httpsPort;
        tempSettings.languages = $tw.Yjs.settings.languages;
        tempSettings.languagesPath = $tw.Yjs.settings.languagesPath;
        tempSettings.logger = $tw.Yjs.settings.logger;
        tempSettings.plugins = $tw.Yjs.settings.plugins;
        tempSettings.pluginsPath = $tw.Yjs.settings.pluginsPath;
        tempSettings.profiles = $tw.Yjs.settings.profiles;
        tempSettings.reverseProxy = $tw.Yjs.settings.reverseProxy;
        tempSettings.rootWikiName = $tw.Yjs.settings.rootWikiName;
        tempSettings.saltRounds = $tw.Yjs.settings.saltRounds;
        tempSettings.saver = $tw.Yjs.settings.saver;
        tempSettings.scripts = $tw.Yjs.settings.scripts;
        tempSettings.servingFiles = $tw.Yjs.settings.servingFiles;
        tempSettings.server = $tw.Yjs.settings.server;
        tempSettings.serverInfo = $tw.Yjs.settings.serverInfo;
        tempSettings.serverKeyPath = $tw.Yjs.settings.serverKeyPath;
        tempSettings.serveWikiOnRoot = $tw.Yjs.settings.serveWikiOnRoot;
        tempSettings.suppressBrowser = $tw.Yjs.settings.suppressBrowser;
        tempSettings.themes = $tw.Yjs.settings.themes;
        tempSettings.themesPath = $tw.Yjs.settings.themesPath;
        tempSettings.tokenPrivateKeyPath = $tw.Yjs.settings.tokenPrivateKeyPath;
        tempSettings.useHTTPS = $tw.Yjs.settings.useHTTPS;
        tempSettings.basePath = $tw.Yjs.settings.basePath;
        tempSettings.wikiPermissionsPath = $tw.Yjs.settings.wikiPermissionsPath;
        tempSettings.wikisPath = $tw.Yjs.settings.wikisPath;
      }
    }
    return tempSettings;
  }
...
class YjsServer extends Yjs {
  constructor () {
    ...
    this.settings = $tw.wiki.getTiddlerData('$:/plugins/joshuafontany/yjs/DefaultSettings',{});
  ...
  */
   ...
   * @param {$tw server state} state
    ...
    Session objects are defined in $:/plugins/joshuafontany/yjs/WSSession.js
  */
          ...
          switch (messageType) {
            ...
            case messageHandshake : {
              ...
              encoding.writeVarString(encoderHandshake, JSON.stringify({
                ...
                settings: $tw.Yjs.getViewableSettings(session.id)
              }));
  ...
  createStateTiddlers (data,wiki) {
    ...
    if (typeof wiki.wikiInfo === 'object') {
      ...
      const fieldsPluginList = {
        title: '$:/state/Yjs/ActivePluginList',
        list: $tw.utils.stringifyList(wiki.wikiInfo.plugins)
      }
      pluginTiddlers['$:/state/Yjs/ActivePluginList'] = fieldsPluginList;
      
      const fieldsThemesList = {
        title: '$:/state/Yjs/ActiveThemesList',
        list: $tw.utils.stringifyList(wiki.wikiInfo.themes)
      }
      pluginTiddlers['$:/state/Yjs/ActiveThemesList'] = fieldsThemesList;
      
      const fieldsLanguagesList = {
        title: '$:/state/Yjs/ActiveLanguagesList',
        list: $tw.utils.stringifyList(wiki.wikiInfo.languages)
      }
      pluginTiddlers['$:/state/Yjs/ActiveLanguagesList'] = fieldsLanguagesList;
    }
    const message = {
      type: 'saveTiddler',
      wiki: data.wiki,

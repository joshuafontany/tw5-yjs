/*\
title: $:/plugins/joshuafontany/yjs/WSSession.js
...
messageHandlers[messageHandshake] = (encoder, decoder, session, doc, emitSynced, messageType) => {
  ...
  // Update the settings
  session.settings = handshakeData.settings;
  // Start a heartbeat
  session.heartbeat();
  // Start a sync
 ...
 class WebsocketSession extends observable_js.Observable {
  ...
  constructor (options) {
    ...
    if(this.client) {
      ...
      this.settings = $tw.wiki.getTiddlerData("$:/config/joshuafontany/yjs/WSSession",{});
      this.doc = options.doc; // Required Y.doc reference
      this.awareness = options.awareness || new awarenessProtocol.Awareness(options.doc); // Y.doc awareness
